...
def structures_to_doublets(
    hits: pd.DataFrame = None, sps: SpacepointStorage = None, ds: DoubletStorage = None
):
    doublets = []
    for i, sp in enumerate(ds.spmIdx):
        inner_indexes = ds.inner[
            ds.innerStart[i] : ds.innerStart[i + 1 if i + 1 < len(ds.spmIdx) else -1]
        ]
        doublets += [(sps.idsp[i], sps.idsp[sp]) for i in inner_indexes]
        outer_indexes = ds.outer[
            ds.outerStart[i] : ds.outerStart[i + 1 if i + 1 < len(ds.spmIdx) else -1]
        ]
...
def cli(out=None, score=True, hits_path=None):
    ...
    if score:
        ...
        print(
            f"DBLETS SCORE -- precision {p * 100}%, recall: {r * 100}% (missing doublets: {len(ms)})"
        )
