...
def filter_doublets(hits, doublets, max_holes):
    ...
    hits["volayer"] = hits[["volume_id", "layer_id"]].apply(
        lambda serie: Volayer.get_index(serie.tolist()), axis=1
    )
    doublets["span"] = (
        hits.volayer.get(doublets.end).values - hits.volayer.get(doublets.start).values
    )
...
def cli(max_holes, hits_path):
    ...
    print(
        f"Discarded {n_discarded} doublets out of {len(doublets)} ({(n_discarded/len(doublets))*100:.3f}%)."
    )
    # save
