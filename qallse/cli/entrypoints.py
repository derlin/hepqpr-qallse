...
class GlobalOptions:
    ...
    def dw(self):
        if self._dw is None:
            self._dw = DataWrapper.from_path(self.path)
        return self._dw
...
@click.option("-i", "--hits-path", help="[required] Path to the hits file.")
@click.option(
    "-o",
    "--output-path",
    default=None,
...
@click.option(
    "-c", "--cls", default="qallse_d0", metavar="module_name", help="Model to use."
)
@click.option(
    "-e",
    "--extra",
...
@cli.command("qbsolv")
@click.option(
    "-q", "--qubo", default=None, metavar="filepath", help="Path a the pickled QUBO."
)
@click.option(
    "-dw",
    "--dwave-conf",
...
def cli_qbsolv(ctx, qubo, dwave_conf, verbosity, logfile, extra):
    ...
    try:
        if qubo is None:
            qubo = ctx.get_output_path("qubo.pickle")
...
@cli.command("neal", help="Sample a QUBO using neal.")
@click.option(
    "-q",
    "--qubo",
    default=None,
...
@click.option(
    "-s", "--seed", default=None, type=int, metavar="int", help="Seed to use."
)
@click.pass_obj
...
def cli_plot(ctx, response, dims, mode):
    ...
    This uses (https://plot.ly) and the qallse.plotting module to
    ...
    """
    from qallse.plotting import iplot_results, iplot_results_tracks
